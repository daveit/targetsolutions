class BarrelsController < ApplicationController
  ...
  # GET /barrels/1.json
  def show
    @details = Redcarpet::Markdown.new(Redcarpet::Render::HTML).render(@barrel.details)
  end
  ...
  private
    ...
    def barrel_params
      params.require(:barrel).permit(:name, :model, :details, :twist, :due_id, :make_id, :image, :bore, :profile, :material, :qty, :details, :active, :price)
    end
end
